WITH SOURCE AS (SELECT
    EXTRACT(DATE FROM creation_date_datetime_utc) as date,
...
GROUP BY organization_name, month, quarter, year, date)
SELECT     
    {{ dbt_utils.generate_surrogate_key(['date','organization_name']) }} as _pk,
    *

FROM source    
Order by date, organization_name